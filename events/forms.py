...
class EventCategoryForm(forms.ModelForm):
    """Форма для категорий мероприятий"""
    ...
    class Meta:
        ...
        widgets = {
            ...
            'icon': forms.TextInput(attrs={'class': 'form-control', 'placeholder': 'fa fa-calendar'}),
        }
        labels = {
...
class EventForm(forms.ModelForm):
    ...
    class Meta:
        model = Event
        fields = [
            'title', 'short_description', 'description', 'category', 'event_type',
            'start_date', 'end_date', 'location', 'venue', 'online_event', 'online_link',
            'address', 'banner_image', 'thumbnail', 'registration_required',
            'registration_type', 'registration_deadline', 'max_participants',
            'waitlist_enabled', 'is_free', 'price', 'currency', 'tags'
        ]
        widgets = {
            'title': forms.TextInput(attrs={'class': 'form-control', 'placeholder': _('Event title')}),
    ...
    def clean(self):
        ...
        if registration_deadline and start_date and registration_deadline > start_date:
            raise forms.ValidationError(_('Registration deadline cannot be after event start date.'))
...
class EventSpeakerForm(forms.ModelForm):
    ...
    class Meta:
        ...
        fields = [
            ...
            'email', 'website', 'linkedin', 'twitter', 'order'
        ]
        widgets = {
...
class EventSessionForm(forms.ModelForm):
    """Форма для сессий мероприятий"""
    
    class Meta:
        model = EventSession
        fields = [
            'title', 'description', 'session_type', 'start_time', 'end_time',
            'location', 'presentation_url', 'materials_url', 'order'
        ]
        widgets = {
            'title': forms.TextInput(attrs={'class': 'form-control'}),
            'description': forms.Textarea(attrs={'class': 'form-control', 'rows': 3}),
...
class EventRatingForm(forms.ModelForm):
    """Форма для оценки мероприятий"""
    
    class Meta:
        model = EventRating
        fields = [
            'rating', 'content_rating', 'organization_rating', 
            'venue_rating', 'comment'
        ]
        widgets = {
            ...
            'content_rating': forms.Select(attrs={'class': 'form-control'}, choices=[
                (1, '1 - Poor'),
                (2, '2 - Fair'),
                (3, '3 - Good'),
                (4, '4 - Very Good'),
            ...
            'organization_rating': forms.Select(attrs={'class': 'form-control'}, choices=[
                (1, '1 - Poor'),
                (2, '2 - Fair'),
                (3, '3 - Good'),
                (4, '4 - Very Good'),
            ...
            'venue_rating': forms.Select(attrs={'class': 'form-control'}, choices=[
                (1, '1 - Poor'),
                (2, '2 - Fair'),
                (3, '3 - Good'),
                (4, '4 - Very Good'),
