    ...
    fieldsets = (
        ...
        (_('Personal info'), {
            'fields': (
                ...
                'date_of_birth', 'bio', 'avatar', 'country', 'city', 'address'
            )
        }),
        (_('Social media'), {
            'fields': ('website', 'linkedin', 'github', 'telegram')
        }),
        (_('Preferences'), {
...
class EmployerProfileAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('Social Media'), {
            'fields': ('company_linkedin', 'company_telegram')
        }),
        (_('Company Details'), {
            'fields': ('company_size', 'industry', 'founded_year', 'headquarters')
        }),
        (_('Statistics'), {
            'fields': ('jobs_posted', 'total_views')
        }),
        (_('Status'), {
            'fields': ('is_verified',)
        }),
...
class StudentProfileAdmin(admin.ModelAdmin):
    list_display = ('user', 'faculty', 'specialty', 'education_level', 'graduation_year')
    ...
    fieldsets = (
        ...
        (_('Education'), {
            'fields': ('education_level', 'graduation_year', 'gpa')
        }),
        (_('Career Preferences'), {
            'fields': ('desired_position', 'desired_salary', 'work_type')
        }),
        (_('Statistics'), {
            'fields': ('resumes_created', 'jobs_applied')
        }),
        (_('Timestamps'), {
            'fields': ('created_at', 'updated_at')
        }),
...
class AdminProfileAdmin(admin.ModelAdmin):
    list_display = ('user', 'can_manage_students', 'can_manage_employers', 'can_manage_jobs')
...
class HemisAuthAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (_('User Data'), {
            'fields': ('hemis_user_data',)
        }),
        (_('Timestamps'), {
            'fields': ('last_sync', 'created_at')
        }),
        (_('Status'), {
            'fields': ('is_token_valid_display',)
        }),
    )
    ...
    is_token_valid.short_description = _('Token valid')
    
    def is_token_valid_display(self, obj):
        return obj.is_token_valid()
    is_token_valid_display.boolean = True
    is_token_valid_display.short_description = _('Token valid')
