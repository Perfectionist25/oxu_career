...
from django.db.models import Q, Avg  # Убрали Count, так как он не используется
from django.utils import timezone

# Исправленные импорты моделей
from .models import Company, EmployerProfile, Job, JobApplication, Interview, CompanyReview, CandidateNote

# Временные формы (создадим позже)
from .forms import (
    CompanyForm, EmployerProfileForm, JobForm, JobApplicationForm,
    CandidateNoteForm, InterviewForm, CompanyReviewForm, JobSearchForm
)
...
def employer_dashboard(request):
    ...
    # Последние отклики
    recent_applications = applications.select_related('candidate', 'job').order_by('-created_at')[:5]
...
def company_detail(request, pk):
    ...
    # Средний рейтинг - ИСПРАВЛЕНО: убрано models.Avg, так как это не импортировано
...
def employer_required(function):
    def wrapper(request, *args, **kwargs):
        ...
        try:
            ...
            messages.error(request, _('You need to complete your employer profile to access this page.'))
            return redirect('employers:create_employer_profile')
    return wrapper
