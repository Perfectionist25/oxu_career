...
def send_interview_invitation(sender, instance, created, **kwargs):
    ...
    if created:
        subject = _('Interview Invitation - {company}').format(company=instance.application.job.company.name)
        message = _(
            'Dear {candidate_name},\n\n'
            ...
            candidate_name=instance.application.candidate.get_full_name() or instance.application.candidate.username,
            job_title=instance.application.job.title,
            datetime=instance.scheduled_date.strftime('%Y-%m-%d %H:%M'),
            location=instance.location,
            duration=instance.duration,
            interviewer=instance.interviewer.user.get_full_name() or instance.interviewer.user.username,
