...
from .forms import AlumniProfileForm, JobApplicationForm, MentorshipRequestForm, MessageForm
...
def event_register(request, pk):
    ...
    if event.participants.filter(id=alumni.id).exists():
        ...
        if event.max_participants and event.participants.count() >= event.max_participants:
            ...
            messages.success(request, 'Siz tadbirga muvaffaqiyatli ro\'yxatdan o\'tdingiz!')
...
def connection_request(request, alumni_slug):
    ...
    existing_connection = Connection.objects.filter(
        from_user=from_alumni, 
        to_user=to_alumni
    ).exists()
    
    if existing_connection:
        messages.warning(request, 'Siz ushbu bitiruvchiga allaqachon so\'rov yuborgansiz.')
        return redirect('alumni:alumni_detail', slug=alumni_slug)
...
def dashboard(request):
    ...
    try:
        ...
        connection_requests = Connection.objects.filter(to_user=alumni, status='pending')
